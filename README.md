...
- Change all `FinanceManagerBackend` occurrences to the name you want to assign to the project. You can do this by using `rename_project.py` script (or taskfile command `rename`).
...
— Change JWT settings in the `FinanceManagerBackend.Api/appsettings.json` file. (It's better to move the secret key out of the codebase into environment variables, for example).

There are also swagger default displaying parameters that you would want to change at `FinanceManagerBackend.Api/appsettings.json` under the `Swagger` block.
